...
// Даты лидов + исторические записи (чтобы работал фильтр по датам/месяцам)
(function seedLeadDates() {
  const now = new Date();
  const iso = (d) => d.toISOString().slice(0, 10);
  const daysAgo = (n) => { const d = new Date(now); d.setDate(d.getDate() - n); return d; };
  const base = DATA.business.conversations;
  const recent = [0, 0, 1, 20];
  base.forEach((c, i) => { c.date = iso(daysAgo(recent[i] || 0)); });
  const extra = [
  ...
  extra.forEach((e) => {
    base.push({
      ...
      preview: e.preview, date: iso(daysAgo(e.days)),
...
let TARIFFS = [
  { plan: "free", title: "Бесплатный", price_month: 0, trial_days: 14, max_channels: 2, max_leads_per_month: 250, analytics: false, manager_control: false, priority_support: false, onboarding: true, storage_months: 3 },
  { plan: "business", title: "Бизнес", price_month: 2990, trial_days: 0, max_channels: 2, max_leads_per_month: 1000, analytics: false, manager_control: false, priority_support: false, onboarding: true, storage_months: 3 },
  { plan: "premium", title: "Премиум", price_month: 7990, trial_days: 0, max_channels: 7, max_leads_per_month: 3000, analytics: true, manager_control: true, priority_support: false, onboarding: true, storage_months: 12 },
  { plan: "enterprise", title: "Корпоративный", price_month: null, trial_days: 0, max_channels: null, max_leads_per_month: null, analytics: true, manager_control: true, priority_support: true, onboarding: true, storage_months: null },
];
...
function viewLeads() {
  ...
  return `<div class="panel">
    ...
    <table class="table" id="leadsTable">
      <thead><tr><th>Дата</th><th>Кто</th><th>Канал</th><th>Источник</th><th>Запрос</th><th>Статус</th><th>Контакт</th></tr></thead>
...
function leadsRows() {
  const list = getLeads();
  if (!list.length) {
    return `<tr><td colspan="7" class="cell-muted" style="text-align:center;padding:26px">Нет лидов по выбранным фильтрам</td></tr>`;
  }
  return list
    .map((c) => {
      const m = CH_META[c.channel];
...
const PLAN_INFO = {
  Бизнес: {
    price: "2 990 ₽ / мес",
    features: ["2 канала", "До 1 000 лидов/мес", "Единый инбокс", "Подключение «под ключ»"],
  },
  Премиум: {
    price: "7 990 ₽ / мес",
    features: ["7 каналов", "До 3 000 лидов/мес", "Аналитика и воронка", "Контроль менеджеров"],
  },
  Корпоративный: {
    price: "по договору",
...
function openPlanUpgrade() {
  ...
  openModal(
    "Выбор тарифа",
    `<div class="plan-choices">${cards}</div>`,
    `
    <button class="btn-d btn-d-ghost" onclick="closeModal()">Закрыть</button>
  `,
  );
  ...
  return `<div class="panel">
    ...
    <div class="table-scroll">
      <table class="table" id="tariffsTable">
        <thead><tr><th>Тариф</th><th>Цена</th><th>Каналы</th><th>Лиды/мес</th><th>Аналитика</th><th>Контроль</th><th>Приоритет</th><th>Хранение</th><th></th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
  </div>
...
function initDashboard() {
  ...
  const trialBar = IS_TRIAL
    ? `<div class="trial-bar">
        ...
        <button class="trial-bar-btn" onclick="openPlanUpgrade()">Оформить подписку</button>
      </div>`
    : "";
