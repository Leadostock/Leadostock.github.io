...
// ===== LOGIN FORM =====
// Адрес бэкенда. Пусто = тот же домен, что и сайт.
// При отдельном хостинге API укажите, напр.: const API_BASE = 'https://api.lidostok.ru';
const API_BASE = '';
...
async function handleLoginSubmit(event) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // Профиль пользователя (имя, роль, тариф)
      let me = {};
      try {
        const meRes = await fetch(`${API_BASE}/api/auth/me`, { headers: { Authorization: `Bearer ${token}` } });
        if (meRes.ok) me = await meRes.json();
      } catch (_) {}
      // Реальные пользователи — владельцы своего кабинета (роль business).
      // Системная админ-панель (все клиенты) появится с отдельным superadmin-эндпоинтом.
      localStorage.setItem('lidostok_session', JSON.stringify({
        role: 'business', name: me.name || login, login: me.email || login,
        token, plan: me.plan || null, tenant_id: me.tenant_id || null, real: true, at: Date.now()
      }));
...
function handleRegisterSubmit(event) {
  ...
  // ===== ЗДЕСЬ ПОДКЛЮЧИТЬ РЕАЛЬНЫЙ API / БАЗУ ДАННЫХ =====
  // const phone = document.getElementById('regPhone').value.trim();
  // const company = document.getElementById('regCompany').value.trim();
  // fetch('/api/auth/register', {
  //   method: 'POST',
  //   headers: { 'Content-Type': 'application/json' },
  //   body: JSON.stringify({ name, email, phone, company, password, plan: new URLSearchParams(location.search).get('plan') })
  // })
  //   .then(r => r.json())
  //   .then(data => {
  //     if (data.success) { formWrap.style.display = 'none'; success.classList.add('show'); }
  //     else showFormError(errorBox, data.message || 'Не удалось создать аккаунт');
  //   })
  //   .catch(() => showFormError(errorBox, 'Ошибка сервера. Попробуйте позже.'))
  //   .finally(() => { btn.textContent = 'Создать аккаунт'; btn.disabled = false; });

  setTimeout(() => {
    document.getElementById('regEmailDisplay') && (document.getElementById('regEmailDisplay').textContent = email);
